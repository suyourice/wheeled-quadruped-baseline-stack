...
#
# What it does:
#   1. Submits a 3-task array job (maze_train / maze_static / maze_dynamic).
#      Each task uses 4 GPUs: 4 ablations in parallel, then teacher sequentially.
#      All 3 tasks run simultaneously → 12 GPUs total, 3× faster than sequential.
#   2. Submits a plot job with --dependency=afterok on the array job.
#      Runs plot_validation.py once all 3 scenarios complete.
#
...
# 1. Submit the 3-task array job.
ARRAY_JOB_ID=$(sbatch --parsable \
    --export=ALL,OUT_NAME="$OUT_NAME" \
    "$SCRIPT_DIR/validate_go2w.sh" \
    "$@")
echo "[submit] Array job submitted: $ARRAY_JOB_ID (tasks 0-2: maze_train/static/dynamic)"

# 2. Submit the plot job, runs only after all 3 array tasks succeed.
